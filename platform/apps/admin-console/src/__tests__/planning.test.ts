...
const EXPECTED_KEYS = [
  ...
  "live_day1_v1",
  "live_day1_h10_v1",
  "live_day2_attended_v1",
  "live_day2_registered_absent_v1",
  "live_day2_not_registered_v1",
  "live_day2_h10_v1",
  "live_day3_attended_v1",
  "live_day3_registered_absent_v1",
  "live_day3_not_registered_v1",
  "live_day3_h2_v1",
  "live_day3_h10_v1",
  "live_day3_h90_v1",
  "post_replay_v1",
  "post_testimonials_v1",
  "post_closer_v1",
  "post_closer_call_v1",
] as const;
...
describe("TEMPLATE_LABELS", () => {
  ...
  it("all keys end with _v1", () => {
    for (const key of Object.keys(TEMPLATE_LABELS)) {
      expect(key, `${key} should end with _v1`).toMatch(/_v1$/);
    }
  });
...
describe("TEMPLATE_VARS", () => {
  ...
  it("broadcast and reminder templates have {{2}}", () => {
    const keysWithTwo = [
      "countdown_j1_v1",
      "live_day1_v1", "live_day2_attended_v1", "live_day2_registered_absent_v1",
      "live_day2_not_registered_v1", "live_day3_attended_v1",
      "live_day3_registered_absent_v1", "live_day3_not_registered_v1",
      "live_day1_h10_v1", "live_day2_h10_v1", "live_day3_h2_v1", "live_day3_h10_v1",
      "live_day3_h90_v1",
      "post_replay_v1", "post_testimonials_v1", "post_closer_v1", "post_closer_call_v1",
    ];
  ...
  it("broadcast templates for day1/2/3 have {{3}} for heure", () => {
    const keysWithThree = [
      "countdown_j1_v1",
      "live_day1_v1",
      "live_day2_attended_v1", "live_day2_registered_absent_v1", "live_day2_not_registered_v1",
      "live_day3_attended_v1", "live_day3_registered_absent_v1", "live_day3_not_registered_v1",
    ];
  ...
  it("welcome_v1 only uses {{1}} (no URL or time)", () => {
    expect(TEMPLATE_VARS["welcome_v1"]).not.toContain("{{2}}");
  });
});
...
describe("PLANNING_STEPS", () => {
  ...
  it("all template references end with _v1", () => {
    for (const step of PLANNING_STEPS) {
      for (const tpl of step.templates) {
        expect(tpl, `step ${step.step} has non-v7 template ${tpl}`).toMatch(/_v1$/);
      }
    }
  });
  ...
  it("DAY_3 includes h2_v1, h10_v1 and h90_v1 timed templates", () => {
    const day3 = PLANNING_STEPS.find((s) => s.step === "DAY_3")!;
    expect(day3.templates).toContain("live_day3_h2_v1");
    expect(day3.templates).toContain("live_day3_h10_v1");
    expect(day3.templates).toContain("live_day3_h90_v1");
  });
...
describe("JOURNEY_PHASES", () => {
  ...
  it("all template keys end with _v1", () => {
    for (const tpl of allPhaseTpls) {
      expect(tpl.key, `${tpl.key} should end with _v1`).toMatch(/_v1$/);
    }
  });
  ...
  it("Phase 4 (Day 3) has 6 templates including h2_v1 and h90_v1", () => {
    const phase4 = JOURNEY_PHASES[3];
    expect(phase4.templates).toHaveLength(6);
    expect(phase4.templates.map((t) => t.key)).toContain("live_day3_h2_v1");
    expect(phase4.templates.map((t) => t.key)).toContain("live_day3_h90_v1");
  });
