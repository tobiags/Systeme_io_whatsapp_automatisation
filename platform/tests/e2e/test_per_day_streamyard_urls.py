...
def test_post_recap_registered_absent_uses_replay_day3_url(monkeypatch):
    import shared.config.settings as cfg_module
    ...
    assert variables["2"] == "https://replay.example.com/day3"
    assert "3" not in variables
    assert "4" not in variables


def test_post_recap_not_registered_prefers_edition_replay_day3_url():
    edition = _FakeEdition(
        replay_day3_url="https://edition.example.com/day3",
        closer_booking_url="https://booking.example.com/edition",
    )
    variables = _build_variables("Lea", "post_recap_not_registered", edition, "EU")
    assert variables["2"] == "https://edition.example.com/day3"
    assert variables["3"] == "https://booking.example.com/edition"
    assert "4" not in variables


def test_post_replay_v1_uses_replay_day3_url(monkeypatch):
    import shared.config.settings as cfg_module

    monkeypatch.setattr(cfg_module.settings, "replay_day3_url", "https://replay.example.com/day3")
    from services.campaigns.app.main import _build_variables as bv

    variables = bv("Lea", "post_replay_v1", None, "EU")
    assert variables["2"] == "https://replay.example.com/day3"


def test_post_replay_v1_prefers_edition_replay_day3_url():
    edition = _FakeEdition(replay_day3_url="https://edition.example.com/day3")
    variables = _build_variables("Lea", "post_replay_v1", edition, "EU")
    assert variables["2"] == "https://edition.example.com/day3"
